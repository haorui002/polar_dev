function [dec_out] = My_SR_Decoder(N,K, dec_in, WQ_LIST,dddd)
    ...
    SR_struct = SR_struct';
    dec_out = decoder(dec_in, N, K, code_struct, idx_fzn,SR_struct,source_len,dddd);
end
...
function [dec_out_all] = decoder(llr_in, N, Kr, node_type_structure, idx_fzn,SR_struct,source_len,dddd)
    %--------------------------------------------------------------------------
    % 输入参数：
    %         llr_in：               信道llr
...
%             xnor_result = ~xor(xor(llr_hard(1:M), llr_hard(M+1:end)),C(c_in_idx,1));
        ...
        switch type
            case -1 % SR_node
                sr_x = SR_decode(P(p_out_idx,1),SR_struct{i_node},node_type_structure(i_node ,4),source_len,mean_value_all*dddd);
                for i = 1 : M        
...
function [SR_X] = SR_decode(LLR,SR_struct,type_source,source_len,mean_value)
    ...
    %剪枝
    valid_LLR_position = abs(LLR) > mean_value; 
